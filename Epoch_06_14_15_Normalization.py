...
Data1 = fits.open("../Data/lcn701020_x1dsum.fits")
#File says that the date obs was 6/14/15 7:15:17
Data2 = fits.open("../Data/lcn701010_x1dsum.fits")
...
#From mathmatica, the polynomial intersects around 1167.21 and 1199.84
#Remember to change this every time if point selection changes
#------------------------------------------------------------------------------
#For Grating 1
Normal_TFS1=[]
for i in range(0,find_index(TWL1,closest_value(TWL1,1163.5289))):                #First part, First order poly
    Normal_TFS1.append(TFS1[i]/(best_fit_poly_1[0]*(TWL1[i])+ best_fit_poly_1[1]))
for i in range(find_index(TWL1,closest_value(TWL1,1163.5289)),len(TWL1)):        #Second part, Second order poly
    Normal_TFS1.append(TFS1[i]/(best_fit_2ndpoly_1[0]*(TWL1[i])**2+ best_fit_2ndpoly_1[1]*(TWL1[i])+ best_fit_2ndpoly_1[2]))
#For Grating 2
Normal_TFS2=[]
for i in range(0,find_index(TWL2,closest_value(TWL2,1199.70394))):                #First part, Second Order Polynomial
    Normal_TFS2.append(TFS2[i]/(best_fit_2ndpoly_2[0]*(TWL2[i])**2+ best_fit_2ndpoly_2[1]*(TWL2[i])+ best_fit_2ndpoly_2[2]))
for i in range(find_index(TWL2,closest_value(TWL2,1199.70394)),len(TWL2)):        #Second part, First Order Polynomial
